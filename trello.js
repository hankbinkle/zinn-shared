...
async function trelloDel(path) {
  const url = `${API_BASE}${path}${path.includes('?') ? '&' : '?'}key=${TRELLO_KEY}&token=${TRELLO_TOKEN}`;
  const res = await fetch(url, {
    method: 'DELETE',
  });
  if (!res.ok) throw new Error(`Trello DELETE ${path} failed: ${res.status}`);
  return res.json();
}
 ...
 */
/**
 * Delete a checkitem from a card's checklist.
 * @param {string} cardId - Card ID
 * @param {string} checkItemId - Checkitem ID
 */
async function deleteCheckitem(cardId, checkItemId) {
  var cls = await trelloGet('/cards/' + cardId + '/checklists');
  if (cls.length === 0) throw new Error('No checklists on card ' + cardId);
